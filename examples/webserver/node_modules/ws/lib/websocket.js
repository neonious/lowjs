...
var WebSocket = /** @class */ (function (_super) {
     ...
     * @private
     */
    WebSocket.prototype.setSocket = function (socket, head, maxPayload) {
     ...
     * @private
     */
    WebSocket.prototype.emitClose = function () {
    ...
    WebSocket.prototype.close = function (code, data) {
        ...
        this._sender.close(code, data, !this._isServer, function (err) {
            ...
            if (_this._socket.writable) {
                if (_this._closeFrameReceived)
                    _this._socket.end();
                //
                // Ensure that the connection is closed even if the closing handshake
                // fails.
                //
                _this._closeTimer = setTimeout(_this._socket.destroy.bind(_this._socket), closeTimeout);
            }
        });
    };
    /**
     ...
     * @public
     */
    WebSocket.prototype.ping = function (data, mask, cb) {
        ...
        if (this.readyState !== WebSocket.OPEN) {
            var err = new Error("WebSocket is not open: readyState " + this.readyState + " " +
     ...
     * @public
     */
    WebSocket.prototype.pong = function (data, mask, cb) {
        if (typeof data === 'function') {
            cb = data;
            data = mask = undefined;
        }
...
function initAsClient(address, protocols, options) {
    ...
    if (protocolVersions.indexOf(options.protocolVersion) === -1) {
        throw new RangeError("Unsupported protocol version: " + options.protocolVersion + " " +
    ...
    if (options.perMessageDeflate) {
        ...
        options.headers['Sec-WebSocket-Extensions'] = extension.format((_a = {},
    ...
    if (options.handshakeTimeout) {
        req.setTimeout(options.handshakeTimeout, function () { return abortHandshake(_this, req, 'Opening handshake has timed out'); });
    }
    req.on('error', function (err) {
        if (_this._req.aborted)
            return;
        req = _this._req = null;
    ...
    req.on('response', function (res) {
        if (_this.emit('unexpected-response', req, res))
            return;
        abortHandshake(_this, req, "Unexpected server response: " + res.statusCode);
    });
    req.on('upgrade', function (res, socket, head) {
        _this.emit('upgrade', res);
        //
        ...
        if (res.headers['sec-websocket-accept'] !== digest) {
            abortHandshake(_this, socket, 'Invalid Sec-WebSocket-Accept header');
            return;
        }
        ...
        if (protError) {
            abortHandshake(_this, socket, protError);
            return;
        }
        if (serverProt)
            _this.protocol = serverProt;
        if (perMessageDeflate) {
            try {
                var extensions = extension.parse(res.headers['sec-websocket-extensions']);
                if (extensions[PerMessageDeflate.extensionName]) {
                    perMessageDeflate.accept(extensions[PerMessageDeflate.extensionName]);
                    _this._extensions[PerMessageDeflate.extensionName] = perMessageDeflate;
                }
            }
            catch (err) {
                abortHandshake(_this, socket, 'Invalid Sec-WebSocket-Extensions header');
                return;
            }
        }
        _this.setSocket(socket, head, options.maxPayload);
    });
}
/**
